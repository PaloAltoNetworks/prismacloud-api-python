...
class ContainersPrismaCloudAPICWPPMixin:
    ...
    def containers_list_read(self, image_id=None, query_params=None):
        if image_id:
            containers = self.execute_compute_paginated('GET', 'api/v1/containers?imageId=%s' % image_id, query_params=query_params, paginated=True)
        else:
            containers = self.execute_compute_paginated('GET', 'api/v1/containers?', query_params=query_params, paginated=True)
        return containers
