...
class PrismaCloudUtility(object):
    ...
    def get_settings(self, args):
        ...
        if args.username is None and args.password is None and args.uiurl is None:
            ...
            self.error_and_exit(400, 'Access Key (--username), Secret Key (--password), and API/UI Base URL (--uiurl) are all required.')
        else:
            settings['apiBase']   = self.normalize_api_base(args.uiurl)
    ...
    def read_settings_file(self, settings_file_name=None):
        ...
        if settings['settings_file_version'] != DEFAULT_SETTINGS_FILE_VERSION:
    ...
    def write_settings_file(self, args):
        ...
        settings = {}
        settings['settings_file_version'] = DEFAULT_SETTINGS_FILE_VERSION
        settings['apiBase']   = self.normalize_api_base(args.uiurl)
        settings['username']  = args.username
        settings['password']  = args.password
        settings['ca_bundle'] = args.ca_bundle
        self.write_json_file(settings_file_name, settings)
    
    # Return user-specified settings file, or the default settings file.
    
    def user_or_default_settings_file(self, settings_file_name=DEFAULT_SETTINGS_FILE_NAME):
    ...
    def normalize_api_base(self, ui_base):
    ...
    def write_json_file(self, file_name, data_to_write):
        file_name_and_path = os.path.join(os.getcwd(), file_name)
        try:
